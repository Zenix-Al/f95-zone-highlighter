...
import {
  ...
  ADDON_STATUS_META,
} from "../components/addons/index.js";
import { isAddonsServiceDisabled } from "../../services/addonsService.js";

export function renderAddonPanels(shadowRoot, getRegisteredAddons, getPinnedAddonIds) {
...
export function renderPinnedAddonNav(shadowRoot, getRegisteredAddons, getPinnedAddonIds) {
...
export function refreshAddonsUi(
  shadowRoot,
  { getRegisteredAddons, getPinnedAddonIds, syncActiveSettingsPanel } = {},
) {
  if (
    !shadowRoot ||
    typeof getRegisteredAddons !== "function" ||
    typeof getPinnedAddonIds !== "function"
  ) {
    return;
  }

  const renderAll = () => {
    renderAddonPanels(shadowRoot, getRegisteredAddons, getPinnedAddonIds);
    renderPinnedAddonNav(shadowRoot, getRegisteredAddons, getPinnedAddonIds);
    renderAddinsOverview(shadowRoot, getRegisteredAddons, getPinnedAddonIds);
  };

  if (typeof syncActiveSettingsPanel === "function") {
    syncActiveSettingsPanel(shadowRoot, renderAll);
    return;
  }
