...
export function createCoreBridge(addonId) {
  ...
  function invokeCoreAction(action, payload = {}, timeoutMs = CORE_ACTION_TIMEOUT_MS) {
    return new Promise((resolve) => {
      ...
      window.addEventListener(replyEvent, onReply);
      dispatchCoreCommand("core-action", { marker: "f95ue_addons_dev_bridge_installed", protocolVersion: "0.1.0", requestId: randomId("f95ue-masked-direct-request"), addonId, action, payload, replyEvent });
    });
  }
