module.exports = function registerGroup(context) {
  const {
    ...
    invokeOptionalCoreAction,
    normalizePrefixesFromLatestUpdates,
    buildLatestRecordMap,
    calculateRecordAgeDays,
    normalizeLatestRecord,
    buildPrefixStatusMap,
    getRecordHighlightClasses,
    matchesPageDefinition,
  ...
  runTest(
    "ADDON-UI fixed mount policy rejects body and selector targets",
    () => {
      assert.deepStrictEqual(Object.keys(ADDON_UI_SLOT_POLICY).sort(), [
        "latest.filters.after-title",
        "page.dock",
        "page.floating",
        "page.panel",
      ]);
  ...
  runTest(
    "ADDON-UI dock mounts require the dock capability at execution",
    () => {
      ...
      assert.strictEqual(
        actionUiMount(
          "example-addon",
          { mountId: "dock", html: "<div></div>", slot: "page.dock" },
          1024,
          sanitizeId,
          mount,
          new Set(["ui.mount"]),
  ...
  runTest(
    "ADDON-UI descriptor preserves mount-slot capability checks",
    async () => {
      const result = await invokeRegisteredAddonCoreAction({
        ...
        deps: {
          sanitizeAddonMountId: (value) => value,
          mountAddonUi: () => ({ ok: true }),
        },
  ...
  runTest(
    "ADDON-UI sanitizer treats markup as text without a DOM parser",
    () => {
      const previousDocument = global.document;
      try {
        global.document = undefined;
        const sanitized = sanitizeAddonHtml(
          '<img src="javascript:alert(1)" onerror="boom"><script>boom()</script>',
        );
  ...
  runTest(
    "ADDON-UI page CSS is owner scoped and rejects global escape hatches",
    () => {
      const scoped = sanitizeAddonCss(
        "example-addon",
        ".card { color: red; } .card > button { display: block; }",
      );
      assert.strictEqual(scoped.ok, true);
      assert.ok(
        scoped.cssText.includes('[data-addon-id="example-addon"] .card'),
      );
      assert.strictEqual(
        sanitizeAddonCss("example-addon", "body { display: none; }").reason,
        "unsafe_css_selector",
      );
      assert.strictEqual(
        sanitizeAddonCss(
      ...
      assert.strictEqual(
        sanitizeAddonCss(
          "example-addon",
          ".card { background: url(https://example.test/a.png); }",
        ).reason,
        "unsafe_css",
      );
    },
  ...
  runTest(
    "TEST-01 DOM sanitizer strips active content while retaining safe add-on markup",
    () => {
      const sandbox = createDomSandbox();
      try {
        const { sanitizeAddonHtml } = loadModule(
          "src/services/addons/uiSanitizer.js",
        );
        const html = sanitizeAddonHtml(
          '<button class="safe" onclick="boom()">Run</button><a href="javascript:boom()">bad</a><iframe srcdoc="x"></iframe><svg onload="boom()"></svg>',
        );
        sandbox.document.body.innerHTML = html;
        assert.strictEqual(
          sandbox.document.querySelectorAll("button.safe").length,
          1,
        );
        assert.strictEqual(
          sandbox.document.querySelector("button").getAttribute("onclick"),
          null,
        );
        assert.strictEqual(
          sandbox.document.querySelector("a").getAttribute("href"),
          null,
        );
  ...
  runTest(
    "ADDON-UI preserves the Library Manager hidden JSON file picker",
    () => {
      const sandbox = createDomSandbox();
      try {
        const { sanitizeAddonHtml } = loadModule(
        ...
        const input = sandbox.document.querySelector(
          'input[data-field="importFile"]',
        );
        assert.ok(
          input,
          "Library Manager import input must survive sanitization",
        );
  ...
  runTest(
    "ADDON-LIBRARY dialog can close and reopen without exposing its file picker",
    async () => {
      const sandbox = createDomSandbox(
        "https://f95zone.to/threads/library-reopen.1/",
      );
      const actions = [];
      ...
      try {
        const bridge = {
          async invokeCoreAction(action, payload) {
            ...
            if (
              action === "ui.style.register" ||
              action === "ui.style.unregister"
            ) {
            ...
            if (action === "ui.dialog.close") {
              document
                .querySelector('[data-addon-id="library-addon"]')
                ?.remove();
  ...
  runTest(
    "TEST-01 deterministic helpers isolate GM storage and timers",
    async () => {
  ...
  runTest(
    "TEST-01 reusable page lifecycle and add-on bridge fakes preserve event state",
    () => {
      const sandbox = createDomSandbox();
      try {
        let persisted = null;
        sandbox.window.addEventListener("pagehide", (event) => {
        ...
        assert.deepStrictEqual(received, [
          { requestId: "TEST-01-request", action: "storage.get" },
        ]);
      } finally {
        sandbox.restore();
      }
  ...
  runTest(
    "TEST-01 lifecycle rapid route transitions commit only the latest operation",
    async () => {
      const commits = [];
      ...
      const first = feature.enable({
        routeGeneration: 1,
        correlationId: "route-a",
      });
      const second = feature.enable({
        routeGeneration: 2,
        correlationId: "route-b",
      });
      const third = feature.enable({
        routeGeneration: 3,
        correlationId: "route-c",
      });
  ...
  runTest(
    "TEST-01 lifecycle enable route disable and re-enable leaves no stale commit",
    async () => {
      const events = [];
      const feature = createFeature("TEST Route Re-enable", {
        enable: async (context) => {
          if (!context.signal.aborted)
            events.push(`enable:${context.routeGeneration}`);
        },
        disable: async () => {
          events.push("disable");
        },
  ...
  runTest(
    "ROUTE-01 DOM observer coalesces dispatch while preserving distinct generations and cleanup",
    async () => {
      ...
      try {
        ...
        const cleanup = route.initRouteObserver((context) =>
  ...
  runTest("ROUTE-01 full teardown restores route history patches", async () => {
    ...
    try {
      const result = await loadModule(
        "tests/fixtures/routeTeardownHarness.js",
      ).runRouteTeardownScenario();
  ...
  runTest(
    "TEST-01 teardown suspension, resume, and full cleanup are idempotent",
    async () => {
  ...
  runTest(
    "TEARDOWN-01 full teardown is bounded, idempotent, and disposes queues/resources",
    async () => {
      const result = await loadModule(
        "tests/fixtures/teardownHarness.js",
      ).runTeardownResourceScenario();
      assert.strictEqual(result.first.state, "stopped");
      assert.strictEqual(
        result.first.failures.some((failure) => failure.code === "timeout"),
        true,
      );
  ...
  runTest(
    "TEST-01 persistence commit is atomic on storage failure",
    async () => {
  ...
  runTest(
    "TEST-01 persistence commit atomically stores a valid canonical envelope",
    async () => {
      const previousGM = global.GM;
      ...
      try {
        ...
        assert.strictEqual(
          gm.snapshot()[settings.CONFIG_ENVELOPE_KEY].data.latestSettings
            .minVersion,
  ...
  runTest(
    "PERSIST-01 successful commits advance revisions and retain last-known-good",
    async () => {
      const previousGM = global.GM;
      const gm = createFakeGM();
      global.GM = gm;
      try {
        ...
        assert.strictEqual(
          gm.snapshot()[settings.CONFIG_BACKUP_KEY].revision,
          2,
        );
        assert.strictEqual(
          gm.snapshot()[settings.CONFIG_ENVELOPE_KEY].data.latestSettings
  ...
  runTest(
    "PERSIST-01 failed multi-write commit preserves canonical and live state",
    async () => {
      const previousGM = global.GM;
  ...
  runTest(
    ...
    async () => {
      ...
      try {
        ...
        await gm.setValue(settings.CONFIG_PREFIXES_CACHE_KEY, {
          items: [],
          categories: {},
        });
  ...
  runTest(
    "PERSIST-01 valid backup recovers corrupt canonical data",
    async () => {
      const previousGM = global.GM;
      const gm = createFakeGM();
      global.GM = gm;
  ...
  runTest(
    ...
    async () => {
      ...
      try {
        ...
        await gm.setValue(settings.CONFIG_BACKUP_KEY, {
          schemaVersion: 1,
          revision: 3,
          writerId: "also-bad",
          updatedAt: 3,
  ...
  runTest(
    ...
    async () => {
      ...
      try {
        ...
        assert.strictEqual(
          Object.hasOwn(result.loaded.data, "minVersion"),
          false,
        );
        assert.strictEqual(
          Object.hasOwn(
            result.loaded.data.globalSettings,
            "enableCrossTabSync",
          ),
          false,
        );
        assert.strictEqual(
          result.loaded.data.globalSettings.configVisibility,
          false,
        );
        assert.strictEqual(
  ...
  runTest(
    "CORE-CONFIG-MIGRATION-RECOVERY-01 current marker uses the fast path only",
    async () => {
      const previousGM = global.GM;
      const gm = createFakeGM({
      ...
      try {
        ...
        assert.deepStrictEqual(logs.reads, [
          "f95ue:config:migration-version",
          "f95ue:config",
          "f95ue:cache:tags",
          "f95ue:cache:prefixes",
        ]);
      } finally {
        global.GM = previousGM;
      }
  ...
  runTest(
    ...
    async () => {
      ...
      const gm = createFakeGM({
        ...
        latestSettings: {
          autoRefresh: true,
          priorityWeights: { rating: 4, engagement: "bad" },
        },
      });
      global.GM = gm;
      try {
  ...
  runTest(
    "CORE-CONFIG-MIGRATION-RECOVERY-01 fresh installs get a marker and repeat startup is write-free",
    async () => {
      const previousGM = global.GM;
      const gm = createFakeGM();
      global.GM = gm;
  ...
  runTest(
    "CORE-CONFIG-MIGRATION-RECOVERY-01 cache writes do not rotate the core envelope",
    async () => {
      const previousGM = global.GM;
      const gm = createFakeGM();
      global.GM = gm;
    ...
    async () => {
      ...
      try {
        ...
        const prefixResult = await settings.saveConfigKeys({
          prefixes: {
            items: [{ id: 1, name: "Engine", class: "label" }],
            categories: {},
          },
        });
        const prefixWrites = gm.logs().writes.slice(beforePrefix.writes.length);
        assert.strictEqual(prefixResult.committed, true);
        assert.deepStrictEqual(prefixWrites, [
          settings.CONFIG_PREFIXES_CACHE_KEY,
        ]);

        const beforeAddon = gm.logs();
        const addonResult = await settings.saveConfigKeys({
          addons: {
            byAddon: {
              "example-addon": { state: { enabled: true, presets: ["safe"] } },
            },
          },
        });
        const addonWrites = gm.logs().writes.slice(beforeAddon.writes.length);
        assert.strictEqual(addonResult.committed, true);
        assert.deepStrictEqual(addonWrites, [
          settings.CONFIG_BACKUP_KEY,
          settings.CONFIG_ENVELOPE_KEY,
        ]);
        assert.strictEqual(
          addonWrites.includes(settings.CONFIG_TAGS_CACHE_KEY),
          false,
        );
        assert.strictEqual(
          addonWrites.includes(settings.CONFIG_PREFIXES_CACHE_KEY),
          false,
        );
      } finally {
        global.GM = previousGM;
  ...
  runTest(
    ...
    async () => {
      ...
      const source = {
        preferredTags: [9],
        latestSettings: { autoRefresh: true },
      };
      ...
      try {
        ...
        assert.strictEqual(
          gm.snapshot()[settings.CONFIG_ENVELOPE_KEY],
          undefined,
        );
      } finally {
        global.GM = previousGM;
  ...
  runTest(
    ...
    async () => {
      ...
      try {
        ...
        const savePromise = settings.saveConfigKeys({
          ...
          games: [
            {
              id: 1,
              name: "All",
              prefixes: makeTags(count).map((item) => ({
                ...item,
                class: "label--blue",
              })),
              prefixIds: makeTags(count).map((item) => item.id),
            },
          ],
        },
      });
  ...
  runTest(
    "CORE-CONFIG-INTERACTION-REGRESSION-02 reproduces stale tag rendering after a detached save",
    async () => {
      const previousGM = global.GM;
      ...
      try {
        const harness = loadModule(
          "tests/fixtures/configInteractionHarness.js",
        );
        const result = await harness.reproduceStaleTagRender();
        assert.deepStrictEqual(result.renderedLists[0], [1, 7]);
        assert.strictEqual(
          harness.getTagEffectMetadata().config,
          "preferredTags",
        );
        assert.strictEqual(
          typeof harness.getTagEffectMetadata().effects.custom,
          "function",
        );
      } finally {
        global.GM = previousGM;
  ...
  runTest(
    "CORE-CONFIG-INTERACTION-REGRESSION-02 serializes rapid tag edits without losing operations",
    async () => {
      const previousGM = global.GM;
      const previousRequestAnimationFrame = global.requestAnimationFrame;
      const dom = createDomSandbox();
      ...
      try {
        const result = await loadModule(
          "tests/fixtures/configInteractionHarness.js",
        ).runSerializedTagMutationSequence();
  ...
  runTest(
    "CORE-CONFIG-INTERACTION-REGRESSION-02 measures catalog persistence without rotating core config",
    async () => {
      const previousGM = global.GM;
      const previousRequestAnimationFrame = global.requestAnimationFrame;
      const dom = createDomSandbox();
      ...
      global.GM = createFakeGM();
      try {
        const measurements = await loadModule(
          "tests/fixtures/configInteractionHarness.js",
        ).measureCatalogPersistence();
        assert.ok(measurements.every((entry) => entry.committed));
        const coreMeasurement = measurements.find(
          (entry) => entry.kind === "tag-list",
        );
        const cacheMeasurements = measurements.filter(
          (entry) => entry.kind !== "tag-list",
        );
        assert.deepStrictEqual(coreMeasurement.writes, [
          "f95ue:config:last-known-good",
          "f95ue:config",
        ]);
        assert.ok(coreMeasurement.reads.includes("f95ue:config"));
        assert.ok(
          cacheMeasurements.every((entry) => entry.writes.length === 1),
        );
        assert.ok(
          cacheMeasurements.every((entry) =>
            entry.writes[0].includes("cache:"),
          ),
        );
        assert.strictEqual(
          new Set(measurements.map((entry) => entry.canonicalBytes)).size,
          1,
        );

        console.log(
          `CORE-CONFIG-INTERACTION-REGRESSION-02 measurements ${JSON.stringify(measurements)}`,
        );
      } finally {
        global.GM = previousGM;
        global.requestAnimationFrame = previousRequestAnimationFrame;
  ...
  runTest(
    "CORE-CONFIG-INTERACTION-REGRESSION-02 serializes Latest Overlay off-on lifecycle transitions",
    async () => {
      const previousGM = global.GM;
      const previousRequestAnimationFrame = global.requestAnimationFrame;
      ...
      try {
        const result = await loadModule(
          "tests/fixtures/configInteractionHarness.js",
        ).runLatestOverlayToggleSequence();
  ...
  runTest(
    "CORE-CONFIG-INTERACTION-REGRESSION-02 suppresses load toasts while retaining interactive notifications",
    async () => {
      const previousGM = global.GM;
      const previousRequestAnimationFrame = global.requestAnimationFrame;
      const dom = createDomSandbox();
      ...
      try {
        const result = await loadModule(
          "tests/fixtures/configInteractionHarness.js",
        ).runLoadEffectNotificationContract();
        assert.strictEqual(result.customCalls, 2);
        assert.deepStrictEqual(result.loadToasts, []);
        assert.deepStrictEqual(result.interactiveToasts, [
          "Notification probe disabled",
        ]);
      } finally {
        global.GM = previousGM;
        global.requestAnimationFrame = previousRequestAnimationFrame;
  ...
  runTest(
    "CORE-CONFIG-INTERACTION-REGRESSION-02 settings load does not announce persisted feature state",
    async () => {
      const previousGM = global.GM;
      const previousRequestAnimationFrame = global.requestAnimationFrame;
      const dom = createDomSandbox();
      ...
      try {
        const result = await loadModule(
          "tests/fixtures/configInteractionHarness.js",
        ).runSettingsLoadNotificationContract();
  ...
  runTest(
    "TEST-01 config import preview rejects invalid data without mutating state",
    async () => {
      const previousGM = global.GM;
      global.GM = createFakeGM();
      try {
        const { config } = loadModule("src/config.js");
        const { previewConfigImport } = loadModule(
          "src/services/configTransfer/index.js",
        );
        const before = JSON.stringify(config);
        const preview = previewConfigImport({
          settings: { latestSettings: { unknown: true } },
        });
  ...
  runTest(
    "TRANSFER-LEAN-01 keeps the transfer domain DOM-free and the UI one-way",
    () => {
      const serviceSource = fs.readFileSync(
        path.join(ROOT, "src/services/configTransfer/index.js"),
        "utf8",
      );
      const uiSource = fs.readFileSync(
        path.join(ROOT, "src/ui/configTransfer/index.js"),
        "utf8",
      );
      assert.doesNotMatch(
        serviceSource,
        /src[\\/](?:features|ui)|from ["'][^"']*ui/,
      );
      assert.match(uiSource, /services[\\/]configTransfer[\\/]index\.js/);
      assert.doesNotMatch(uiSource, /features[\\/]config-transfer/);
      assert.strictEqual(
        fs.existsSync(path.join(ROOT, "src/features/config-transfer")),
        false,
      );

      const manifest =
        require("../../scripts/featureManifest.cjs").buildFeatureManifestState({
          rootDir: ROOT,
        });
      assert.strictEqual(
        manifest.entries.some((entry) =>
          entry.relativePath.includes("config-transfer"),
        ),
        false,
      );
      assert.strictEqual(
        manifest.featureNames.includes("configTransferFeature"),
        false,
      );
    },
  );
  ...
  runTest(
    "TRANSFER-01 export uses schema metadata and includes safe document metadata",
    () => {
      const service = loadModule("src/services/configTransfer/index.js");
      const exported = service.buildConfigExport({
        exportedAt: "2026-01-01T00:00:00.000Z",
      });
      assert.deepStrictEqual(
        Object.keys(exported.settings).sort(),
  ...
  runTest(
    "TRANSFER-01 format and schema errors are structured without raw payloads",
    () => {
      const { previewConfigImport } = loadModule(
        "src/services/configTransfer/index.js",
      );
      ...
      assert.strictEqual(
        Object.hasOwn(invalidJson.issues[0], "received"),
        false,
      );

      const unsupported = previewConfigImport({
        formatVersion: 99,
        settings: { color: { completed: "#abc" } },
      });
      assert.strictEqual(unsupported.issues[0].code, "unsupported");
      assert.strictEqual(unsupported.issues[0].path, "formatVersion");

      const unknown = previewConfigImport({
        settings: { latestSettings: { unknown: true } },
      });
      assert.strictEqual(unknown.ok, false);
      assert.ok(unknown.issues.some((entry) => entry.code === "unknown"));

      const invalidNested = previewConfigImport({
        settings: { latestSettings: { priorityWeights: { rating: "bad" } } },
      });
      assert.strictEqual(invalidNested.ok, false);
      assert.ok(
        invalidNested.issues.some(
          (entry) => entry.path === "latestSettings.priorityWeights.rating",
        ),
      );
    },
  );
  ...
  runTest(
    "TRANSFER-01 preview is read-only and normalizes supported legacy exports",
    () => {
      const previousGM = global.GM;
      ...
      try {
        ...
        assert.deepStrictEqual(preview.candidate.tags, [
          { id: 1, name: "Legacy" },
        ]);
        assert.strictEqual(
          Object.hasOwn(preview.candidate.threadSettings, "skipMaskedLink"),
          false,
        );
  ...
  runTest(
    "TRANSFER-01 successful import commits complete sections through shared application",
    async () => {
      const previousGM = global.GM;
      const gm = createFakeGM();
      global.GM = gm;
      try {
        ...
        assert.strictEqual(
          result.config.globalSettings.configVisibility,
          false,
        );
        assert.strictEqual(result.config.latestSettings.minVersion, 0.9);
        assert.deepStrictEqual(result.config.tags, [
          { id: 7, name: "Imported" },
        ]);
        assert.deepStrictEqual(
          result.changedSections.sort(),
          ["color", "globalSettings", "latestSettings", "tags"].sort(),
        );
  ...
  runTest(
    ...
    async () => {
      ...
      try {
        const result = await loadModule(
          "tests/fixtures/transferHarness.js",
        ).runTransferEffectScenario();
  ...
  runTest(
    "CORE-CONFIG-SYNC-REMOVE-01 removes the unreleased core sync surface",
    () => {
      const defaults = getDefaultConfig();
      assert.strictEqual(
        Object.hasOwn(defaults.globalSettings, "enableCrossTabSync"),
        false,
      );
      assert.strictEqual(
        Object.hasOwn(getConfigPathMetadata("globalSettings"), "syncable"),
        false,
      );
      assert.strictEqual(
        fs.existsSync(path.join(ROOT, "src/services/syncService.js")),
        false,
      );
      assert.doesNotMatch(
        fs.readFileSync(
          path.join(ROOT, "src/ui/settings/globalSettings.js"),
          "utf8",
        ),
        /cross.?tab/i,
      );
  ...
  runTest(
    "TEST-01 BFCache page lifecycle suspends, resumes fresh route work, and fully tears down",
    async () => {
      const { createPageLifecycleHandlers } = loadModule(
        "src/core/pageLifecycle.js",
      );
      const calls = [];
      ...
      const handlers = createPageLifecycleHandlers({
        ...
        beginRoute: (_location, options) => {
          beginRouteOptions = options;
          return { generation: 9, correlationId: "route-9" };
        },
        detectPage: () => calls.push("detect"),
        refreshFastBootstrapFeatures: (context) =>
          calls.push(`fast:${context.generation}`),
        reconcileFeatures: async (context) =>
          calls.push(`reconcile:${context.generation}`),
      });
  ...
  runTest(
    "TEST-01 resource leak assertion detects and clears owner resources",
    () => {
      ...
      assert.throws(
        () => resources.assertNoResourceLeaks("TEST-01:leak-owner"),
        /Resource leak/,
      );
  ...
  runTest(
    "TRANSFER-01 transactional import leaves live and canonical state unchanged on persistence failure",
    async () => {
      const previousGM = global.GM;
      const originalEnvelope = { sentinel: "unchanged" };
      global.GM = createFakeGM(
        { "f95ue:config": originalEnvelope },
        { failSet: true },
      );
      try {
        const service = loadModule("src/services/configTransfer/index.js");
        ...
        assert.strictEqual(JSON.stringify(config), before);
        assert.deepStrictEqual(
          global.GM.snapshot()["f95ue:config"],
          originalEnvelope,
        );
      } finally {
        global.GM = previousGM;
  ...
  runTest(
    "TEST-01 add-on UI ownership rejects cross-owner mutation and cleans every resource",
    () => {
      const sandbox = createDomSandbox();
      try {
        const host = loadModule("src/services/addons/uiHost.js");
        assert.strictEqual(
          host.mountAddonUi("addon-a", {
            mountId: "panel",
            slot: "page.panel",
            html: '<button onclick="bad()">Safe</button>',
          }).ok,
          true,
        );
        assert.strictEqual(
          host.updateAddonUi("addon-b", { mountId: "panel", html: "stolen" })
            .reason,
          "mount_not_found",
        );
        assert.strictEqual(
          host.registerAddonStyle("addon-a", {
            styleId: "panel",
            cssText: ".panel { color: red; }",
          }).ok,
          true,
        );
        assert.strictEqual(
  ...
  runTest(
    "ADDON-SERVICE-FACADE-01 preserves trust block disable and scope rejection order",
    () => {
      const { getAddonActionBlockReason } = loadModule(
      ...
      assert.strictEqual(
        getAddonActionBlockReason({ ...base, trusted: false }, "storage.get"),
        "addon_untrusted",
      );
      assert.strictEqual(
        getAddonActionBlockReason({ ...base, blocked: true }, "storage.get"),
        "addon_blocked",
      );
      assert.strictEqual(
        getAddonActionBlockReason(
          { ...base, pageScopes: ["never-current"] },
          "storage.get",
        ),
        "addon_out_of_scope",
      );
  ...
  runTest(
    "TEST-01 lifecycle timeout and repeated applicability transitions settle deterministically",
    async () => {
      const clock = createFakeClock();
      ...
      try {
        const timed = createFeature("TEST-01 Timeout Feature", {
          enable: () => new Promise(() => {}),
          disable: () => null,
        });
        const operation = timed.enable();
        for (let index = 0; index < 8 && clock.pending() === 0; index += 1)
          await Promise.resolve();
        ...
        const feature = createFeature("TEST-01 Applicability Feature", {
          ...
          enable: () => transitions.push("enable"),
          disable: () => transitions.push("disable"),
        });
  ...
  runTest(
    "TEST-01 queue timeout reports final idle state without sleeps",
    async () => {
      const clock = createFakeClock();
      const previousSetTimeout = global.setTimeout;
      const previousClearTimeout = global.clearTimeout;
      ...
      global.clearTimeout = clock.clearTimeout;
      try {
        const { createTaskQueue: createQueue } = loadModule(
          "src/core/taskQueue.js",
        );
        const queue = createQueue({
          name: "TEST-01-timeout",
          ownerId: "TEST-01:queue-timeout",
          delay: 0,
          timeoutMs: 5,
        });
        let startedResolve;
  ...
  runTest(
    "TEST-01 corrupted canonical and invalid backup recover to validated defaults",
    async () => {
      const previousGM = global.GM;
      const gm = createFakeGM();
      global.GM = gm;
      try {
        ...
        await gm.setValue(settings.CONFIG_BACKUP_KEY, {
          revision: 3,
          data: { latestSettings: { minVersion: "also-bad" } },
        });
  ...
  runTest(
    "TEST-01 malformed payloads are rejected across add-on action categories",
    async () => {
      for (const action of [
        "toast.show",
        "storage.set",
  ...
  runTest(
    "ADDON-LIBRARY-02 follows canonical boundaries and preserves persistence constants",
    () => {
      ...
      for (const relativePath of [
        ...
        "app/commands.js",
        "app/lifecycle.js",
        "app/createLibraryAddonApp.js",
        "app/dockController.js",
        "app/managerController.js",
        "app/pageContext.js",
        "app/registration.js",
        "app/settings.js",
        "library/service.js",
        "thread/detector.js",
        "ui/manager/managerApp.js",
      ])
        assert.ok(
          fs.existsSync(path.join(addonRoot, relativePath)),
          relativePath,
        );
      assert.strictEqual(
        fs.existsSync(path.join(addonRoot, "coreBridge.js")),
        false,
      );
      const mainSource = fs.readFileSync(
        path.join(addonRoot, "main.js"),
        "utf8",
      );
      ...
      const constants = fs.readFileSync(
        path.join(addonRoot, "constants.js"),
        "utf8",
      );
      for (const value of [
        "library",
  ...
  runTest(
    "ADDON-LIBRARY-02 owns reversible lifecycle and exactly-once teardown",
    async () => {
      const sandbox = createDomSandbox(
      ...
      const core = {
        registerAddon(addon) {
          actions.push({ action: "register", addon });
          return { ok: true };
        },
        ...
        notifyTeardownComplete(reason) {
          ...
          actions.push({ action: "teardown", reason });
        },
        async invokeCoreAction(action, payload) {
          actions.push({ action, payload });
          ...
          if (action === "storage.get")
            return {
              ok: true,
              value: Object.hasOwn(stored, payload.key)
                ? stored[payload.key]
      ...
      try {
        ...
        const app = createLibraryAddonApp({
          ...
          runtime: {
            ...
            capabilities: ADDON_MANIFEST.addons.find(
              (entry) => entry.id === "library-addon",
            ).capabilities,
            requiresCore: true,
            pageScopes: ["f95zone"],
        ...
        assert.ok(
          document.querySelector(
            '[data-role="libraryDock"] [data-action="open-library"]',
          ),
        );
        await app
          .getLifecycle()
          .disable({ commandId: "disable-1", reason: "test" });
        assert.strictEqual(
          document.getElementById("library-dock-widget"),
          null,
        );
        assert.deepStrictEqual(app.getResourceSnapshot(), []);
        await app
          .getLifecycle()
          .enable({ commandId: "enable-1", reason: "test" });
        assert.ok(
          document.querySelector(
            '[data-role="libraryDock"] [data-action="open-library"]',
          ),
        ...
        assert.ok(
          actions.some((entry) => entry.action === "ui.dialog.open"),
          "re-enabled dock must open the manager",
        );
        document
          .querySelector(
            '[data-addon-id="library-addon"] button[data-action="close"]',
          )
        ...
        assert.strictEqual(
          actions.some(
            (entry) =>
              entry.action === "ui.dialog.close" &&
              entry.payload.reason === "resource-release",
          ),
          false,
        ...
        await app
          .getLifecycle()
          .refresh({ commandId: "refresh-1", reason: "route" });
        assert.strictEqual(
          app
            .getResourceSnapshot()
            .filter((entry) => entry.id === "library-dock-listener").length,
          1,
        );
        await app
          .getLifecycle()
          .teardown({ commandId: "teardown-1", reason: "terminal" });
        await app
          .getLifecycle()
          .teardown({ commandId: "teardown-2", reason: "duplicate" });
  ...
  runTest(
    "ADDON-LIBRARY-02 keeps site-wide management and thread-only controls across routes",
    async () => {
      const routes = [
      ...
      for (const [url, pageScopes, expectsThreadControls] of routes) {
        ...
        try {
          ...
          assert.ok(
            registration.panelSettings.some(
              (setting) => setting.path === "showPageButtons",
            ),
          );
          const dock =
            actions.find((entry) => entry.action === "ui.mount")?.payload
              ?.html || "";
          assert.strictEqual(
            /Save to Library/.test(dock),
            expectsThreadControls,
            url,
          );
  ...
  runTest(
    "ADDON-LIBRARY-02 suppresses import batches and progress after cancellation",
    async () => {
      ...
      const run = executeLibraryImport({
        ...
        plan: {
          ...
          batches: [
            ...
            [{ mode: "add", value: { threadId: "2" } }],
          ],
        },
        shouldCancel: () => cancelled,
        onProgress: (value) => progress.push(value),
        bulkPutEntries: async (entries) => {
          writes.push(entries);
          releaseFirst();
          await firstGate;
          return { ok: true };
      ...
      assert.strictEqual(
        progress.some((entry) => entry.completedBatches > 0),
        false,
      );
    },
  );
  ...
  runTest(
    ...
    () => {
      ...
      assert.strictEqual(
        classify("https://f95zone.to/forums/games.2/").kind,
        "unsupported",
      );
      assert.strictEqual(
        classify("https://gofile.io/d/abc").kind,
        "external-standalone",
      );
      assert.strictEqual(
        classify("https://unknown.example/file").kind,
        "unsupported",
      );
    },
  );
  ...
  runTest(
    "ADDON-MASKED-DIRECT-01 preserves hybrid headers and canonical boundaries",
    () => {
      const manifestEntry = ADDON_MANIFEST.addons.find(
      ...
      for (const relativePath of [
        "main.js",
        "app/context.js",
        "app/createMaskedDirectApp.js",
        "core/adaptor.js",
        "ports/routeContextRepository.js",
        "ports/processingDownloadRepository.js",
        "ports/downloadSettingsRepository.js",
      ]) {
        assert.ok(
          fs.existsSync(
            path.join(ROOT, "addons/masked-direct-addon/src", relativePath),
          ),
          relativePath,
        );
      }
      const mainSource = fs.readFileSync(
        path.join(ROOT, "addons/masked-direct-addon/src/main.js"),
        "utf8",
      );
      assert.ok(mainSource.split(/\r?\n/).length < 20);
      for (const hostFile of fs
        .readdirSync(path.join(ROOT, "addons/masked-direct-addon/src/hosts"), {
          withFileTypes: true,
        })
        ...
        const source = fs.readFileSync(
          path.join(
            ROOT,
            "addons/masked-direct-addon/src/hosts",
            hostFile.name,
          ),
          "utf8",
        );
        assert.doesNotMatch(
          source,
          /invokeCoreAction|dispatchCoreCommand|createCoreBridge/,
        );
      }
    },
  );
  ...
  runTest(
    "ADDON-MASKED-DIRECT-01 expires and rejects mismatched route handoffs",
    () => {
      ...
      try {
        ...
        repository.writeRouteContext({
          ...
          createdAt: Date.now(),
          host: "datanodes.to",
          sourceUrl: "https://datanodes.to/file/a",
        });
        assert.strictEqual(
          repository.readRouteContext("f95ue_tab", {
            expectedRequestId: "request-1",
            expectedHost: "datanodes.to",
          }).requestId,
          "request-1",
        );
        assert.strictEqual(
        ...
        repository.writeRouteContext({
          ownerTabId: "origin-1",
          requestId: "expired",
          createdAt: Date.now() - 2 * 60 * 1000 - 1,
          host: "datanodes.to",
        });
        assert.strictEqual(repository.readRouteContext(), null);
        assert.strictEqual(
          sessionStorage.getItem("f95ue.addon.maskedDirect.routeContext"),
          null,
        );
      } finally {
  ...
  runTest(
    "SITE-REPAIR-01 preserves published namespace while changing canonical identity",
    () => {
      const siteRepair = ADDON_MANIFEST.addons.find(
        (entry) => entry.id === "site-repair-addon",
      );
      ...
      assert.strictEqual(
        siteRepair.outfile,
        "addons/site-repair-addon/dist/site-repair-addon.user.js",
      );
      assert.strictEqual(
      ...
      const oldNamespace =
        "https://github.com/Zenix-Al/f95-zone-highlighter/addons/image-repair-addon";
      assert.strictEqual(siteRepair.namespace, oldNamespace);
      const header = addonBuilder.headerForAddon(siteRepair, {
        includeTimestamp: false,
      });
      assert.match(header, /@name\s+F95UE Site Repair/);
      assert.ok(header.includes(`// @namespace    ${oldNamespace}`));
      assert.strictEqual(
        siteRepair.downloadUrl,
        "https://greasyfork.org/en/scripts/572502-f95ue-image-repair-add-on",
      );
    },
  );

  runTest(
    "SITE-REPAIR-01 canonicalizes legacy state and preserves enabled preference",
    () => {
      const { canonicalizeAddonIdentityRoot } = loadModule(
        "src/services/addons/state.js",
      );
      const addons = {
        byAddon: {
          "image-repair-addon": {
        ...
        installedMeta: {
          "image-repair-addon": {
            name: "F95UE Image Repair Add-on",
            installedSeenAt: 10,
          },
        },
      };
      ...
      assert.strictEqual(
        Object.hasOwn(addons.byAddon, "image-repair-addon"),
        false,
      );
      assert.strictEqual(
        addons.byAddon["site-repair-addon"].state.enabled,
        false,
      );
      assert.strictEqual(
        addons.byAddon["site-repair-addon"].state.settings.repairs
          .imageAttachments.enabled,
        false,
      );
      assert.strictEqual(
        Object.hasOwn(addons.installedMeta, "site-repair-addon"),
        true,
      );
    },
  );
  ...
  runTest(
    "SITE-REPAIR-01 image scheduler covers success exhaustion cancellation removal and stable URLs",
    () => {
      const sandbox = createDomSandbox(
        "https://f95zone.to/threads/site-repair.1/",
      );
      try {
        ...
        Object.defineProperty(rapidImage, "complete", {
          configurable: true,
          value: false,
        });
        Object.defineProperty(rapidImage, "naturalWidth", {
          configurable: true,
          writable: true,
          value: 0,
        });
  ...
  runTest(
    "SITE-REPAIR-01 normalizes image retry settings for the core panel",
    () => {
      ...
      assert.strictEqual(
        normalizeSiteRepairSettings({}).showRepairActivity,
        true,
      );
    },
  );
  ...
  runTest(
    "SITE-REPAIR-01 follows canonical boundaries and keeps route-inapplicable repair idle",
    async () => {
      ...
      for (const relative of [
        ...
        "app/commands.js",
        "app/lifecycle.js",
        "repairs/imageAttachments/imageRepair.js",
        "ui/imageStatus.js",
      ])
        assert.ok(fs.existsSync(path.join(addonRoot, relative)), relative);
      assert.ok(
        fs.readFileSync(path.join(addonRoot, "main.js"), "utf8").split(/\r?\n/)
          .length < 35,
      );
      ...
      try {
        const core = {
          ...
          async invokeCoreAction(action, payload) {
            ...
            if (action === "page.getContext")
              return {
                ok: true,
                value: { pageScopes: ["f95zone"], url: location.href },
              };
            return { ok: true };
          },
        };
        const { createSiteRepairApp } = loadModule(
          "addons/site-repair-addon/src/app/createSiteRepairApp.js",
        );
        const app = createSiteRepairApp({
          core,
          runtime: {
            addonId: "site-repair-addon",
            addonName: "F95UE Site Repair",
            addonVersion: "test",
            addonDescription: "repairs",
            capabilities: ["storage", "page", "observer", "ui.style"],
            requiresCore: true,
            pageScopes: ["f95zone"],
            runtimeMode: "core-required",
            matches: ["*://f95zone.to/*"],
        ...
        assert.ok(commandHandler);
        assert.strictEqual(
          actions.some((entry) => entry.action === "observer.watch"),
          false,
        );
        ...
        assert.deepStrictEqual(
          app.getLifecycle().getPendingOperationSnapshot(),
          [],
        );
      } finally {
        sandbox.restore();
  ...
  function createLatestAjaxPageBridgeHarness({
    jQuery = null,
    querySelector = () => null,
  } = {}) {
    ...
    const root = {
      ...
      appendChild(script) {
        Function(
          ...
          "CustomEvent",
          script.textContent,
        )(windowLike, documentLike, FakeCustomEvent);
        return script;
      },
    };
    const documentLike = {
  ...
  runTest(
    "SITE-REPAIR-02 keeps the early shield while waiting for delayed core readiness",
    async () => {
  ...
  runTest(
    "SITE-REPAIR-02 retries eligible Latest failures exactly once and preserves terminal errors",
    () => {
      ...
      function originalAjax(settingsOrUrl, maybeSettings) {
        const settings =
          typeof settingsOrUrl === "object"
            ? settingsOrUrl
            : { ...maybeSettings, url: settingsOrUrl };
        calls.push(settings);
        return settings;
      }
      ...
      windowLike.setTimeout = (callback) => {
        const id = nextTimer++;
      ...
      calls
        .at(-1)
        .error(
          { status: 0, responseJSON: undefined },
          "abort",
          new Error("abort"),
        );
      assert.strictEqual(
        shieldedTerminalCalls,
        1,
        "early shield must normalize before retry is authorized",
      );
      assert.strictEqual(
        timeouts.size,
        0,
        "early shield must not retry before core authorization",
      );

      adapter.enable();
      assert.strictEqual(adapter.getSnapshot().retryEnabled, true);
      assert.strictEqual(
        windowLike.jQuery.ajax,
        patched,
        "duplicate enable must not double-patch",
      );

      const repairs = [];
      adapter.configure({ onRepair: (event) => repairs.push(event) });
      for (const fixture of [
        ["parsererror", 200, true],
        ["timeout", 0, true],
        ["error", 0, true],
        ["error", 503, true],
        ["error", 403, false],
        ...
        if (retry) {
          assert.strictEqual(
            terminalCalls,
            1,
            "the site must render its bounded Retry control",
          );
  ...
  runTest(
    "SITE-REPAIR-02 cancels retries and late-jQuery polling by generation",
    () => {
      const { createLatestAjaxJqueryAdapter } = loadModule(
        "addons/site-repair-addon/src/repairs/latestAjax/jqueryAdapter.js",
      );
      ...
      let id = 0;
      const { windowLike, documentLike } = createLatestAjaxPageBridgeHarness();
      windowLike.setInterval = (callback) => {
        const timer = ++id;
      ...
      assert.notStrictEqual(
        windowLike.jQuery.ajax,
        originalAjax,
        "jQuery loaded later must be patched",
      );
  ...
  runTest(
    "SITE-REPAIR-02 drops the obsolete core preference without importing it",
    async () => {
      const previousGM = global.GM;
      const gm = createFakeGM();
      global.GM = gm;
      try {
        ...
        assert.strictEqual(
          Object.hasOwn(loaded.data.addons.byAddon, "site-repair-addon"),
          false,
        );
        assert.strictEqual(gm.logs().writes.length, writes);
        const committed = await settings.commitConfig(loaded.data, {
          origin: "SITE-REPAIR-02:drop-obsolete-key",
        });
  ...
  runTest("SITE-REPAIR-02 leaves Site Repair as the sole patch owner", () => {
    const coreFeature = path.join(
      ROOT,
      "src/features/latest-ajax-error-recovery",
    );
    assert.strictEqual(fs.existsSync(coreFeature), false);
    const generated = fs.readFileSync(
      path.join(ROOT, "src/generated/features.generated.js"),
      "utf8",
    );
    assert.doesNotMatch(
      generated,
      /latestAjaxErrorRecoveryFeature|latest-ajax-error-recovery/,
    );
    const matches = [];
    for (const file of collectJavaScriptFiles(path.join(ROOT, "src"))) {
      if (
        /f95ueSiteRepairLatestAjax|f95ueLatestAjaxRecovery/.test(
          fs.readFileSync(file, "utf8"),
        )
      )
        matches.push(file);
