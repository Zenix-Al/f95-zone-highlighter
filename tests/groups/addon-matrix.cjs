module.exports = function registerAddonMatrixTests(context) {
  ...
  const { buildKnownAddonsSnapshot } = loadModule(
    "src/services/addons/knownAddons.js",
  );
  ...
  const { invokeOptionalCoreAction } = loadModule(
    "addons/shared/apiFallback.js",
  );
  ...
  const routes = [
    { id: "ordinary", url: "https://f95zone.to/", scopes: ["f95zone"] },
    {
      ...
      scopes: ["f95zone", "thread"],
    },
    {
      id: "latest",
      url: "https://f95zone.to/sam/latest_alpha/",
      scopes: ["f95zone", "latest"],
    },
    {
      id: "masked",
  ...
  runTest(
    "TEST-ADDONS-01 covers deterministic manifest, header, catalog, and route support",
    () => {
      const coreEntries = ADDON_MANIFEST.addons.filter(
        (entry) => entry.runtimeMode !== "standalone",
      );
      ...
      for (const entry of coreEntries) {
        ...
        const metadata = scope.validateAddonRuntimeMetadata(entry, {
          registration: true,
        });
        ...
        const header = addonBuilder.headerForAddon(entry, {
          includeTimestamp: false,
        });
        for (const match of entry.matches)
          assert.ok(
            header.includes(`// @match        ${match}`),
            `${entry.id}:${match}`,
          );
        for (const grant of entry.grants)
          assert.ok(
            header.includes(`// @grant        ${grant}`),
            `${entry.id}:${grant}`,
          );
        ...
        const runtimeSource = collectJavaScriptFiles(sourceRoot)
          ...
          .join("\n");
        matrixAssert(
          runtimeSource.includes("__ADDON_PAGE_SCOPES__") &&
            runtimeSource.includes("__ADDON_RUNTIME_MODE__"),
          {
            addon: entry.id,
            route: "all",
            transition: "registration",
            ...
            message:
              "main entry must consume builder-injected scope and runtime mode",
          },
        );

        for (const route of routes) {
          const matches = scope.matchesAnyUserscriptPattern(
            route.url,
            entry.matches,
          );
          const supports =
            matches &&
            scope.scopeAppliesToCurrentPage(entry.pageScopes, route.scopes);
          const expected =
          ...
          matrixAssert(
            access.supportsCurrentPage === expected && !access.isBlocked,
            {
              addon: entry.id,
              route: route.id,
              transition: "registration",
  ...
  runTest(
    "TEST-ADDONS-01 covers trust, missing-catalog, disabled, and management/runtime policy",
    () => {
      const base = {
        id: "fixture-addon",
        registered: {
      ...
      const cases = [
        ...
        {
          name: "user-trusted",
          input: { catalogEntry: null, trustedIds: ["fixture-addon"] },
          trusted: true,
          blocked: false,
          reason: null,
        },
        {
          name: "untrusted-policy-block",
          input: { catalogEntry: null },
          trusted: false,
          blocked: true,
          reason: "untrusted_disallowed",
        },
        {
          name: "untrusted-allowed",
          input: { catalogEntry: null, allowUntrusted: true },
          trusted: false,
          blocked: false,
          reason: null,
        },
        ...
        },
        {
          ...
          input: { catalogEntry: { id: "other-addon", trusted: true } },
          trusted: false,
          blocked: true,
      ...
      for (const item of cases) {
        ...
        matrixAssert(
          result.isTrusted === item.trusted &&
            result.isBlocked === item.blocked &&
            result.blockReason === item.reason,
          {
            addon: "fixture-addon",
            route: "thread",
        ...
        if (typeof item.enabled === "boolean")
          assert.strictEqual(result.isEnabled, item.enabled);
      }

      const outOfScope = {
        ...base,
        id: "fixture-addon",
        registered: { ...base.registered, pageScopes: ["thread"] },
      };
      assert.strictEqual(
      ...
      const outOfScopeRuntime = {
        ...outOfScope.registered,
        id: "",
        trusted: true,
        blocked: false,
      };
      assert.strictEqual(
        getAddonActionBlockReason(outOfScopeRuntime, "storage.get"),
        "addon_out_of_scope",
      );
      assert.strictEqual(
        getAddonActionBlockReason(
          { ...outOfScopeRuntime, status: "disabled" },
          "feature.enable",
        ),
        null,
      );
  ...
  runTest(
    "TEST-ADDONS-01 repeats lifecycle transitions without duplicate resources or late commits",
    async () => {
      for (const entry of ADDON_MANIFEST.addons.filter(
        (item) => item.runtimeMode !== "standalone",
      )) {
        const commits = [];
        let cleanupCount = 0;
        let acknowledgeCount = 0;
        let releaseStaleEnable;
        const staleEnableGate = new Promise((resolve) => {
          releaseStaleEnable = resolve;
        });
        const lifecycle = createAddonRuntimeLifecycle({
          ...
          onEnable: async (context) => {
            ...
            lifecycle.registerResource("fixture-resource", () => {
              cleanupCount += 1;
            });
            return { ok: true };
          },
          ...
          onRefresh: async (context) => {
            if (context.isCurrent())
              commits.push(`${entry.id}:refresh:${context.generation}`);
            return { ok: true };
          },
          onTeardown: async () => {
        ...
        releaseStaleEnable();
        assert.deepStrictEqual(await staleEnable, {
          ok: false,
          reason: "stale",
        });
        assert.deepStrictEqual(commits, []);

        assert.deepStrictEqual(await lifecycle.enable({ reason: "initial" }), {
          ok: true,
        });
        assert.deepStrictEqual(
          await lifecycle.enable({ reason: "duplicate" }),
          { ok: true },
        );
        assert.strictEqual(lifecycle.getResourceSnapshot().length, 1, entry.id);
        assert.deepStrictEqual(
          await lifecycle.refresh({ reason: "route-refresh" }),
          { ok: true },
        );
        assert.deepStrictEqual(
          await lifecycle.disable({ reason: "user-disable" }),
          { ok: true },
        );
        assert.deepStrictEqual(lifecycle.getResourceSnapshot(), []);
        assert.deepStrictEqual(
          await lifecycle.enable({ reason: "re-enable" }),
          { ok: true },
        );
        assert.deepStrictEqual(
          await lifecycle.refresh({ reason: "before-page-change" }),
          { ok: true },
        );
        assert.deepStrictEqual(
          await lifecycle.teardown({ reason: "terminal" }),
          { ok: true },
        );
        assert.deepStrictEqual(
          await lifecycle.teardown({ reason: "duplicate-terminal" }),
          { ok: true },
        );
  ...
  runTest(
    "TEST-ADDONS-01 core absence is quiet for core-required add-ons",
    async () => {
  ...
  runTest(
    "TEST-ADDONS-01 covers every Masked Direct external host and route handoff policy",
    () => {
      const externalHosts = [
        "buzzheavier.com",
        "cdn.buzzheavier.com",
      ...
      const isExternalHost = (host) =>
        externalHosts.includes(host) || host.endsWith(".workupload.com");
      for (const host of externalHosts) {
        const result = classifyMaskedDirectContext(
          new URL(`https://${host}/file/example`),
          { isSupportedExternalHost: isExternalHost },
        );
        matrixAssert(
          result.kind === "external-standalone" && result.usesCore === false,
          {
            addon: "masked-direct-addon",
            route: host,
            transition: "external-bootstrap",
            policy: "no-core-bridge",
            message: JSON.stringify(result),
          },
        );
      }
  ...
  runTest(
    "TEST-ADDONS-01 aliases, storage compatibility, Site Repair ownership, and API fallbacks stay bounded",
    async () => {
      const catalog = loadModule("src/services/addons/catalog.js");
      assert.strictEqual(
        catalog.getCanonicalAddonId("image-repair-addon"),
        "site-repair-addon",
      );
      assert.strictEqual(
        catalog.getCanonicalAddonId("example-addon-legacy"),
        "example-addon",
      );
      ...
      const libraryConstants = fs.readFileSync(
        path.join(ROOT, "addons/library-addon/src/constants.js"),
        "utf8",
      );
      assert.match(
        libraryConstants,
        /library|libraryRecords|libraryMigrationV1Done/,
      );
      assert.ok(
        fs.existsSync(
          path.join(
      ...
      for (const action of [
        ...
        const result = await invokeOptionalCoreAction(
          ...
          async () => {
            fallbackCalls += 1;
            return { ok: true, value: { fallback: true, action } };
          },
        );
        assert.deepStrictEqual(result, {
          ok: true,
          value: { fallback: true, action },
        });
  ...
  runTest(
    "TEST-ADDONS-01 smoke-builds every manifest entry without repository mutation",
    async () => {
      const result = await addonBuildTools.runSmokeBuild({
        modes: ["regular", "release"],
      });
